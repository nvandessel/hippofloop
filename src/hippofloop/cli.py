...
def main() -> None:
    ...
    train_parser.add_argument("--config", default="configs/default.yaml", help="Training config YAML")
    ...
    eval_parser.add_argument("--config", default="configs/default.yaml", help="Training config YAML")
...
def _cmd_train(args: argparse.Namespace) -> None:
    ...
    result = trainer.train(train, val, args.config)
...
def _cmd_eval(args: argparse.Namespace) -> None:
    ...
    cleaner = DecisionCleaner()
    cleaned = cleaner.clean(entries)

    formatter = SftFormatter()
    pairs = formatter.format(cleaned)

    _, _, test = formatter.split(
        pairs,
        train_ratio=config.train_split,
        val_ratio=config.val_split,
        seed=config.seed,
    ...
    # Lazy import — eval with the actual model requires GPU
    print(f"Evaluating {args.model} on {len(test)} test examples...")
    print("(GPU-dependent eval not yet implemented — use explore for data analysis)")
