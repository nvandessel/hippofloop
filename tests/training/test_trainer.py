...
def mock_config() -> TrainingConfig:
    return TrainingConfig(
        ...
        max_seq_length=4096, weight_decay=0.01, bf16=True,
...
def test_prepare_dataset_preserves_message_structure(mock_config: TrainingConfig, train_data: list[SFTPair]):
    trainer = UnslothTrainer(mock_config)
    dataset = trainer.prepare_dataset(train_data)
